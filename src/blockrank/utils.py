...
def format_ranking_prompt_mistral(
    ...
    if block_order == "doc_first":
        if query_in_instruction:
            prompt_parts = doc_blocks + [f"{instruction}\n{doc_intro}\nQuery: {query}\n\n{final_section}"]
        else:
            prompt_parts = doc_blocks + [f"{instruction}\n{doc_intro}\n\n{final_section}"]
    else:
        prompt_parts.extend(doc_blocks)
...
def format_ranking_prompt_qwen(
    ...
    if block_order == "doc_first":
        if query_in_instruction:
            prompt_parts = doc_blocks + [f"<Instruct>: {instruction}\n{doc_intro}\n<Query>: {query}\n{final_section}"]
        else:
            prompt_parts = doc_blocks + [f"<Instruct>: {instruction}\n{doc_intro}\n{final_section}"]
    else:
        prompt_parts.extend(doc_blocks)
    ...
    # Keep answer marker in the prompt so the completion can start with "[".
    # prompt_parts.append("<think> </think>\n<answer>:") # 将<think>放入user prompt
    prompt = sep.join(prompt_parts)
    return prompt
...
def create_conversation_format_qwen(
    ...
    # if answer_ids:
    #     assistant_content = f"{sep}[" + (', '.join([str(x) for x in answer_ids]) + f"]")
    # else:
    #     assistant_content = f"{sep}["
    # return [
    #     # {"role": "system", "content": "Judge whether the Document meets the requirements based on the Query and the Instruct provided."},
